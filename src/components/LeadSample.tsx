...
interface LeadSampleTableProps {
  leads: { node: Lead }[]
}
...
const LeadSampleTable: React.FC<LeadSampleTableProps> = ({ leads }) => {
  ...
  const onPageChange = (cp: { selected: number }) => {
    setActiveLeads(
      leads.slice(
...
interface LeadSampleRowProps {
  lead: Lead
}

const LeadSampleRow: React.FC<LeadSampleRowProps> = ({ lead }) => {
  const { data } = lead
    ...
    <StaticQuery
      ...
      render={data => {
        const leads = data.allMongodbLeadGeekLeads.edges.filter(
          ...
          }) => lead.node.data.date >= prevTwoWeeks
        )
